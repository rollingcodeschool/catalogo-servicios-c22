...
const Menu = ({ usuarioLogueado, setUsuarioLogueado }: MenuProps) => {
  ...
  const navLinkStyles = ({ isActive }: { isActive: boolean }) =>
    `block py-2 px-3 transition-colors duration-200 md:p-0 ${
      isActive ? "text-blue-500 font-semibold" : "text-zinc-300 hover:text-blue-400"
    }`;
    ...
    <nav className="bg-zinc-950 border-b border-zinc-800 text-zinc-100 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="shrink-0 flex items-center gap-2 text-xl tracking-wider">
            ...
            <Link className="font-bold" to={'/'}>CODE</Link>
          </div>
      ...
      >
        <div className="px-4 pt-2 pb-6 space-y-2">
          <NavLink to="/" className={navLinkStyles} onClick={() => setIsMenuOpen(false)}>
            Inicio
          </NavLink>
